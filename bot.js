...
client.on("message", message => {
    if (message.content === (prefix + "help")) {
     const embed = new Discord.RichEmbed() 
         ...
         _users : عدد اعضاء السيرفر
     
         _channels : عدد رومات السيرفر
    
         _avatar : تاخد صورتك او صورة عضو محدد
    
         _roll : تسحب من عدد عشوائي
         _roll 11 : مثال امر رول يحسب من عدد 1 ل 11 و يكتبلك كم طلعت النتيجة
...
client.on('message', message => {
    if (message.content.startsWith("_url")) {
...
if (message.content === '_inv') {
      message.author.send('https://discordapp.com/oauth2/authorize?client_id=515185172921057297&scope=bot&permissions=1').catch(e => console.log(e.stack));
...
client.on("guildMemberAdd", member => {
        if(member.guild.id === "492672255105433620") {  // ايدي السيرفر
  const channel = member.guild.channels.find('id', '505045499133558785'); //ايدي الروم
if (!channel) return;
channel.send(`**<@${member.user.id}> Welcome To __King Community__  ** ❤️ `)  
}});
client.on("guildMemberRemove", member => {
        if(member.guild.id === "492672255105433620") { 
  const channel = member.guild.channels.find('id', '505045499133558785'); 
if (!channel) return;
  channel.send(`**${member.user.tag} Left The Server  ** 😭 `) 
}});
client.on('guildMemberAdd', member=> {
    member.addRole(member.guild.roles.find("name","- Kings Communtiy -")); //اسم الرتبة
    });
